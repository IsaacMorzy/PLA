  ...
  return (
    <footer className="relative z-30 overflow-hidden border-t border-white/8 bg-[#0c0a08] text-white">
      ...
      <div className="relative mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 lg:py-20">
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-3">
              {[
                ...
                <div key={item.label} className="rounded-[1.4rem] border border-white/10 bg-black/20 px-4 py-4">
                  <p className="text-[11px] uppercase tracking-[0.24em] text-white/55">{item.label}</p>
            ...
            <div className="rounded-[2rem] border border-white/10 bg-white/[0.045] p-7 backdrop-blur-xl sm:p-8">
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <h3 className="mt-3 font-display text-3xl text-white">{TRUST_COPY.newsletterTitle}</h3>
                </div>
              ...
              <div className="grid gap-8 md:grid-cols-[0.75fr_1.25fr]">
                ...
                <div className="grid gap-8 sm:grid-cols-3">
                  {Object.entries(FOOTER_LINKS).map(([title, links]) => (
                    <div key={title}>
                      <h4 className="text-sm font-semibold uppercase tracking-[0.26em] text-white/58">{title}</h4>
                      <ul className="mt-4 space-y-3">
                        {links.map((link) => (
                          <li key={link.href}>
                            <Link
                              href={link.href}
                              className="text-sm text-white/68 transition-colors duration-300 hover:text-[#f1ddab] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#d4a853]/70 focus-visible:ring-offset-2 focus-visible:ring-offset-[#0c0a08]"
                            >
                              {link.label}
                            </Link>
                          </li>
        ...
        <div className="mt-8 flex flex-col gap-4 border-t border-white/8 pt-6 text-sm text-white/62 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-[11px] uppercase tracking-[0.22em] text-white/68">
              Solana-native
            </span>
          </div>
        </div>
...
function SocialLink({
  ...
  return (
    <a
      ...
      aria-label={label}
      className="inline-flex h-11 w-11 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.04] text-white/72 transition-all duration-300 hover:-translate-y-0.5 hover:border-[#d4a853]/30 hover:bg-[#d4a853]/10 hover:text-[#f1ddab] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#d4a853]/70 focus-visible:ring-offset-2 focus-visible:ring-offset-[#0c0a08]"
    >
      {icon}
    </a>
  );
